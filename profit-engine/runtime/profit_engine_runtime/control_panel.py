...
<div class="section"><h2>P0 · Ручной Search Profit Control</h2><div class="rows"><div class="row"><b>DILIVOX | SEARCH | PROFIT ENGINE</b><span>Search only</span><span>WeeklySpendLimit фиксирует Owner</span><span class="pill warn">BUILD FIRST</span></div><div class="row"><span>Keyword bids</span><span>Shadow controller → guarded writer</span><span>K5 — главный judge</span><span class="pill locked">WRITE LOCKED</span></div></div></div>
<div class="footer">Секреты: macOS Keychain · bind: 127.0.0.1 · provider writes из панели: 0</div>
</div><script>
function p(state){if(state==='PASS'||state==='ACTIVE'||state==='READ_MODEL_READY'||state==='READY_FOR_CANDIDATE_EVALUATION')return 'ok';if(String(state).includes('ERROR')||String(state).includes('REWORK'))return 'bad';return 'warn'}
function pill(s){return `<span class="pill ${p(s)}">${s||'—'}</span>`}
function esc(x){return String(x??'—').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}
function render(s){
 document.getElementById('system').textContent=s.state||'—'; document.getElementById('updated').textContent=s.updated_at?new Date(s.updated_at).toLocaleString():'—';
 document.getElementById('yanlink').textContent=s.metrica_yan_monetization_ready?'PASS':'WAIT'; document.getElementById('yanlinkState').textContent=s.metrica_yan_monetization_ready?'Монетизация читается':'Ожидаем активацию/данные';
 let money=s.money||[]; document.getElementById('money').innerHTML=money.length?money.map(m=>`<div class="row"><b>${esc(m.label)}</b><span>Расход: ${esc(m.direct_spend_rub)} ₽</span><span>РСЯ: ${esc(m.metrica_attributed_yan_revenue_rub)} ₽</span><span>K5: ${esc(m.k5_observed)} ${pill(m.state)}</span></div>`).join(''):'<div class="row"><span>Money preflight ещё не доступен</span><span></span><span></span><span></span></div>';
 let campaigns=s.campaigns||[]; document.getElementById('campaigns').innerHTML=campaigns.length?campaigns.map(c=>`<div class="row"><b>${esc(c.label)}</b><span>${esc(c.campaign_id)}</span><span>${pill(c.state)}</span><span>${esc(c.status)}</span></div>`).join(''):'<div class="row"><span>Нет свежего inventory</span></div>';
 let gs=(s.metrica_goals&&s.metrica_goals.goals)||[]; document.getElementById('goals').innerHTML=gs.length?gs.map(g=>`<div class="row"><b>${esc(g.name)}</b><span>${esc(g.identifier)}</span><span>${pill(g.state)}</span><span>${esc(g.role)}</span></div>`).join(''):'<div class="row"><span>Цели ещё не проверены</span></div>';
}
